...
- `src/face_model_core/config.py`: immutable training configuration and validation.
...
- `src/face_model_core/train.py`: end-to-end training loop with AdamW, AMP support, and checkpointing.
...
- `src/face_model_core/cli.py`: `train`, `build-gallery`, and `infer` commands with bounded threshold parsing and mixed precision opt-out.
